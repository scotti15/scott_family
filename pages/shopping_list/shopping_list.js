document.addEventListener("DOMContentLoaded", () => {
  ...
  const shoppingListTableBody = document.querySelector(
    "#shoppingListTable tbody"
  );
  ...
  const populateDropdown = async (selectEl, url, placeholder = "Select...") => {
    ...
    data.sort((a, b) =>
      a[Object.keys(a)[1]].localeCompare(b[Object.keys(b)[1]])
    );
  ...
  const loadShoppingList = async () => {
    ...
    data.forEach((row) => {
      ...
      tr.innerHTML = `
                <td>${
                  row.IsBargain == "1" ? "💰 " + row.ItemName : row.ItemName
                }</td>
                <td>${row.BrandName}</td>
                <td>${row.PlaceName}</td>
  ...
  // Helper: format a cents-per-unit number to max 4 decimals, strip trailing zeros
  ...
  /**
   * row should contain:
   *  - Price (in cents, numeric)
   *  - Amount (numeric)
   *  - ConversionToBase (numeric, e.g. 453.592 for lb)
   *  - UnitType ('solid'|'liquid'|'each')
   *  - UnitName (optional, e.g. 'lb', 'each')
   *
   * Returns a formatted string like "1.0517 ¢ / g" or "129 ¢ / each"
   */
  const computeNormalizedPrice = (row) => {
    const price = parseFloat(row.Price);
    const amount = parseFloat(row.Amount);
    const conv = parseFloat(row.ConversionToBase ?? 1);
    const utype = (row.UnitType ?? "").toLowerCase();
    const uname = (row.UnitName ?? "").toLowerCase();

    if (isNaN(price) || isNaN(amount) || amount <= 0) return "";

    // Treat 'each' either by UnitType or UnitName
    if (utype === "each" || uname === "each") {
      // price is total cents for the whole amount; price per each = price / amount
      const perEach = price / amount;
      return `${formatCents(perEach)} ¢ / each`;
    }

    // For solids/liquids: convert amount to base (g or ml) using ConversionToBase
    const amountBase = amount * (isNaN(conv) ? 1 : conv);
    if (amountBase <= 0) return "";

    const perUnit = price / amountBase; // cents per g or ml
    const displayUnit =
      utype === "solid" ? "g" : utype === "liquid" ? "ml" : "unit";

    return `${formatCents(perUnit)} ¢ / ${displayUnit}`;
  };
  ...
  document
    .querySelector("#shoppingListTable")
    .addEventListener("click", async (e) => {
      ...
      await populateDropdown(
        document.getElementById("editBrandSelect"),
        "get_brands.php",
        "Select Brand"
      );
      ...
      await populateDropdown(
        document.getElementById("editPlaceSelect"),
        "get_places.php",
        "Select Place"
      );
      ...
      await populateDropdown(
        document.getElementById("editUnitSelect"),
        "get_units.php",
        "Select Unit"
      );
  ...
  const priceHistoryContainer = document.getElementById(
    "priceHistoryContainer"
  );
  const priceHistoryTableBody = document.querySelector(
      ...
      const normalizedPrice =
        row.UnitType === "each"
          ...
          : (
              ...
              (parseFloat(row.Amount) * parseFloat(row.ConversionToBase))
            ).toFixed(4);
  ...
  bargainCheckbox.addEventListener("change", () => {
    normalizedPriceBadge.style.display = bargainCheckbox.checked
      ? "inline-block"
      : "none";
